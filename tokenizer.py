...
class BPETokenizer:
    ...
    def _get_chunk_boundaries(self, input_path: str, parallelism: int) -> list[int]:
    ...
    def _pretokenize(
        ...
        boundaries = self._get_chunk_boundaries(input_path, parallelism)
        args = [
            (input_path, start, end, special_pat) 
            for start, end in zip(boundaries[:-1], boundaries[1:])
        ]
        with Pool(parallelism) as p:
            pretokens = p.starmap(self._pretokenize_chunk, args)
    ...
    def _find_merges(
        ...
        def apply_merge(rule: tuple[int, int], children: set[tuple[int, int]]):
            merged_token = []
        ...
        while len(vocab) < vocab_size:
            ...
            for token, weight in tokens.items():
                for pair in zip(token[:-1], token[1:]):
                    hit_pair = pairs.get(pair, MergePair(weight=0, children=set()))
                    
                    hit_pair.weight += weight
                    hit_pair.children.add(token)
                    pairs[pair] = hit_pair
            
            max_pair = max(pairs, key=lambda x: (pairs[x].weight, x))

            apply_merge(max_pair, pairs[max_pair].children)
    ...
    def train_bpe(
        ...
        vocab, merges = self._find_merges(pretokens, vocab_size, special_tokens)
